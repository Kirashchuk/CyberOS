...
export async function runReindex(vaultPath: string): Promise<void> {
  ...
  const metrics = buildMetrics(executeResults, 2, 180, 0.12);
  ...
  const state = {
    status: pausedForMaintenance ? "paused_maintenance" : "ready",
    lastReindex: new Date().toISOString(),
    notes: "Phase 1 index built from markdown sources with observability controls",
    correlation,
  ...
  const structuredLogPath = await writeStructuredRunLog(vaultPath, "reindex", correlation, [
    ...
    },
    {
      level: "info",
      event: "index.rebuilt",
      message: `Indexed ${filesIndexed} files and ${factsIndexed} facts`,
      source_ref: "rebuildFactsIndex",
      attributes: {
        filesIndexed,
        factsIndexed
      }
